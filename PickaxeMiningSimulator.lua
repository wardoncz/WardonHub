local Player, Rayfield, Click, comma, Notify, CreateWindow, CurrentVersion = loadstring(game:HttpGet("https://raw.githubusercontent.com/wardoncz/WardonHub/main/Variables.lua"))()
			...
			for i,v in pairs(SelectedArea:GetChildren()) do
				if v and v:FindFirstChild("Type") and (v.Type.Value:split("Ore")[2] == Rayfield.Flags.Ore.CurrentOption or v.Type.Value == Rayfield.Flags.Ore.CurrentOption or Rayfield.Flags.Ore.CurrentOption == "All") then
					...
					if not ChildAdded then
						print("[WardonHub] Debug: Mined "..v.Name.." in "..math.round(tick() - StartTime).." seconds")
					else
						ChildAdded = false
					end
				end
...
Main:CreateButton({
	...
	Callback = function()
		for _,v in pairs({"release", "wow500likes", "likes1000thx", "update1", "newgame", "thx2500likes", "update2", "visits2m"}) do
			...
			print("[WardonHub] Debug: Redeemed code "..v)
			task.wait(2)
		end
	end,
})
	...
	for i,v in pairs(Items) do
		if v.Amount >= 5 then
			...
			for e = 1, 5 do
				...
				print("[WardonHub] Debug: Selected Giant ID "..Items[i].Ids[e])
			end
...
Items:CreateToggle({
	...
	Info = "Turns 5 of the same pet into a rainbow!",
	CurrentValue = false,
	Flag = "RainbowPets",
	Callback = function(Value)	end,
})

local function Rainbow(ToRainbow, ScrollingFrame, Remote)
	local Items = {}

	for i,v in pairs(Player.PlayerGui.Main.Prompts.Rainbow[ScrollingFrame]:GetChildren()) do
		if v.ClassName == "ImageButton" then
			if not Items[v.Icon.Image] then
				Items[v.Icon.Image] = {Amount = 1, Ids = {v.Name}}
			else
	...
	for i,v in pairs(Items) do
		if v.Amount >= 5 then
			...
			for e = 1, 5 do
				Selected[Items[i].Ids[e]] = true
				print("[WardonHub] Debug: Selected Rainbow ID "..Items[i].Ids[e])
			end
...
task.spawn(function()
	while task.wait() do
		if Rayfield.Flags.RainbowPets.CurrentValue then
			Rainbow("Pet", "ScrollingFramePets", "RequestRainbowPet")
		end
	end
end)

Items:CreateToggle({
	Name = "🛠 Auto Rainobw Pickaxes",
	Info = "Turns 5 of the same pickaxe into a Rainbow!",
	CurrentValue = false,
	Flag = "RainbowPicks",
	Callback = function(Value)	end,
})
...
task.spawn(function()
	while task.wait() do
		if Rayfield.Flags.RainbowPicks.CurrentValue then
			Rainbow("Pickaxe", "ScrollingFrame", "RequestRainbow")
		end
	end
end)
